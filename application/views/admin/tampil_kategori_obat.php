                        ...
                        <div class="col-xs-4">
                            <div class="box">
                                <div class="box-header">
                                    ...
                                    <br>
                                    <form action="<?= base_url('obat/tambah_kategori_obat') ?>" method="post">
...
<script>
    $(document).ready(function() {
        ...
        function tampil_data() {
            $.ajax({
                type: 'ajax',
                url: '<?= base_url() ?>obat/tampil_data_kategori_obat',
                async: false,
                dataType: 'json',
                success: function(data) {
                    ...
                    for (i = 0; i < data.length; i++) {
                        html += '<tr>' +
                            ...
                            '<td>' + "<button class='glyphicon glyphicon-trash btn btn-danger' onclick='return hapus(" + data[i].id + ")'></button>" + '</td>' +
                            '</tr>';
                    }
                    $('#show_data').html(html);
                }
    ...
    function hapus(id) {
        if (confirm('Hapus Kategori Obat?')) {
            window.location = 'hapus_kategori_obat?id=' + id + 'refresh';
        } else {
            function hapus(id) {
                window.location = 'tampil_kategori_obat', 'refresh';
            }
        }
    }

    function edit(id) {
        window.location = 'edit_kategori?id=' + id;
    }
</script>